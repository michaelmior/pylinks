...
def do_macro(parser, token):
    ...
    for arg in args:
        if not r_valid_arg_name.match(arg):
            raise template.TemplateSyntaxError, \
                    "Argument '%s' to macro '%s' contains illegal " + \
                    "characters. Only alphanumeric characters and '_' " + \
                    "are allowed." % (arg, macro_name)
...
def do_usemacro(parser, token):
    ...
    if (len(values) != len(macro.args)):
        raise template.TemplateSyntaxError, \
                "Macro '%s' was declared with %d parameters " + \
                "and used with %d parameter" % (
                    macro_name,
                    len(macro.args),
                    len(values)
                )
