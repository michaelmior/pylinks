...
class CategoryModelTests(TestCase):
    def test_category_sort(self):
        ...
        self.assertEqual(["Test 1", "Test 2"], [str(c) for c in Category.objects.all()])
...
class ListViewTests(TestCase):
    def setUp(self):
        ...
        link = Link.objects.create(
            title="Google", url="https://www.google.ca", visits=1
        )
        ca.links.add(link)

        link = Link.objects.create(title="GitHub", url="https://github.com", visits=2)
        ca.links.add(link)

        link = Link.objects.create(title="UW", url="https://uwaterloo.ca", visits=3)
        cb.links.add(link)
